...
class CategoryController extends Controller
{
  ...
  //Ver estos permisos para que el bot funcione correctamente
    public function __construct(){
        $this->middleware('api.auth',['except'=>['index','show1','show','store2','update2']]);
    }
    ...
    public function store2(Request $request){
        $params_array=(json_decode(file_get_contents("php://input"), true));
        ...
        if(!empty($params_array)){
            ...
            if($validate->fails()){
                $data = [
                    'code'=>404,
                    'status'=>'error',
                    'message'=>'No se ha guardado la categoria falta name'
                ];
            }else{
                $category = new Category();
                $category->name = $params_array['name'];
